...
1. In both Nodes 2.0 and Legacy Canvas, add **Reference Loader** from `reference / loader`, then add **Reference Loader Export Prompt for LLM**, **Reference Loader Raw Outputs**, and **Reference Loader Start/End Frames** from `reference / output`. Connect `references` to all three output nodes and confirm the full board and Prompt editor render without console errors.
...
11. Create a `#` dialogue block and inspect the literal tags in Raw. Queue Export Prompt for LLM and verify its compact strict YAML contains active tag-to-caption mappings, an ordered `generation_directives` mapping, resolved media tags, dialogue tags, and `source_video` only for enabled video-derived Audio paired with an enabled Video. Confirm it contains no top-level `prompt`, `prompt_schema_preset`, media payload, or absolute path. Save the workflow, restart ComfyUI, restore it, and verify state/order/captions/edit recipes/prompt parts.
