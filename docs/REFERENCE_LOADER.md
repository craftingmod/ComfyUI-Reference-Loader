...
For an LLM-refined prompt path, connect the LLM STRING to **[Reference Loader] Reference Prompt Cache** as `live_prompt`, and connect the same `references` bundle. With a matching `cached_hash`, the node returns `cached_prompt` and ComfyUI's lazy scheduler does not evaluate `live_prompt`. `cached_prompt` is intentionally editable for manual refinement; `cached_hash` remains read-only. A stale hash or enabled `force_refresh` requests the live value and stores the resulting prompt and current bundle fingerprint in the workflow widgets. Use `force_refresh` when LLM-side options changed without changing the references.
