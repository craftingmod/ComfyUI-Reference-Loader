...
`Reference Loader` is a ComfyUI V3 node for arranging local image, audio, and video references before another node analyzes or generates from them. It owns file ingestion, independent image/video/audio ordering, raw user captions, enable state, non-destructive edits, and a structured reference-aware prompt. It emits one compact reference bundle containing the execution-relevant prompt snapshot and a canonical fingerprint. `[Reference Loader] Raw Prompt` extracts its compiled STRING, `[Reference Loader] Reference Prompt Cache` avoids reevaluating a connected lazy LLM prompt while that fingerprint is current, `Prompt Live Cache` provides the same pattern for a caller-supplied complete STRING identity, and `[Reference Loader] Export Prompt for LLM` combines the same snapshot with active captions through one connection. It does not load an LLM/VLM or rewrite prompt text itself. `[Reference Loader] Start/End Frames` converts up to two enabled Images into nullable scalar frame outputs for I2V, L2V, FL2V, FL2V_LOOP, and T2V nodes.

Reference Loader and Load Reference Image appear under `reference / loader`; [Reference Loader] Reference Prompt Cache appears under `reference / prompt`; Prompt Live Cache appears under `reference / util`; [Reference Loader] Raw Prompt, [Reference Loader] Export Prompt for LLM, [Reference Loader] Media Outputs, and [Reference Loader] Start/End Frames appear under `reference / output`; and [Reference Loader] MiniMax H3 Wrapper appears under `reference / integration`. A minimal saved graph is available as [`Reference_Loader.json`](../workflows/Reference_Loader.json).
...
The H3 Timeline widget is placed directly after Media, followed by the Subjects & Shots widget and then Prompt. Vue Nodes sizes the Media, H3, and Subjects rows as `max-content` and gives remaining height to Prompt; Legacy Canvas uses each DOM-widget's min/max height contract. Both modes avoid nested scrollbars and prevent spare height from becoming a gap before Prompt.

5. Prompt content is arranged as a stack of title-tag sections. Drag a section header's handle to change the compiled section order, or focus the handle and press `Alt+Up` / `Alt+Down`. Type `@` inside any section for a media mention. Generic permits `#name` to create a Subject in any section. MiniMax H3 Reference permits creation only in `subject_definitions`, then offers the same Subject from `#` in every section. MiniMax H3 Base and Freeform leave `#` as literal text. Section bodies and Raw use native multiline editing: `Enter` creates an ordinary new line and `Shift+Enter` follows the browser's standard `contenteditable` behavior. In the **Add section** row, enter a lowercase pseudo-YAML title such as `integrated_multimodal_description:` directly, or type `/` to choose an alias from the active prompt preset. Use **Raw** to inspect or edit the complete pseudo-YAML text, or **Copy** to place that same compiled Prompt on the clipboard.
...
7. Use **I**, **V**, **VA**, or **A** to include or exclude a card from its matching output channel. **VA** controls whether embedded audio is included in the VIDEO value; **A** independently controls the derived AUDIO value.
8. Use an Audio card's **▶/■** button for audio auditioning, or a VIDEO card's **▶/■** button for a preview of its applied range. The VIDEO-board preview and trim editor follow **VA** and mute playback when it is off; the Audio-board editor remains audible. Open **Edit** to crop/flip an image, optionally extract its foreground with `rembg`, paint an erase/restore keep mask, restore a materialized edit to its immutable original, choose transparent or solid background output, or trim and audition one audio/video item by seconds.
...
For an LLM-refined prompt path, connect the LLM STRING to **[Reference Loader] Reference Prompt Cache** as `live_prompt`, and connect the same `references` bundle. With a matching `cached_ref_hash`, the node returns `cached_prompt` and ComfyUI's lazy scheduler does not evaluate `live_prompt`. Optionally connect a canonical string of LLM-side settings to `invalidate_key`; a changed value also invalidates the prompt cache. For example, use KJ Nodes' **Something To String** and/or string-combine nodes to include a seed, branch name, model, temperature, or other prompt-generation settings. `cached_prompt` is intentionally editable for manual refinement; `cached_ref_hash` and `cached_invalidate_key` remain read-only. A stale reference hash or invalidate key, or enabled `force_refresh`, requests the live value and stores the resulting prompt, current bundle fingerprint, and invalidate-key fingerprint in the workflow widgets. Use `force_refresh` when another LLM-side option is not included in `invalidate_key`.

For a source-independent prompt cache, add **Prompt Live Cache** from `reference / util`. Connect a required complete cache identity to `invalidate_key` and the generated STRING to `live_prompt`. Its editable `cached_prompt` widget is the persisted fallback. Unlike `[Reference Loader] Reference Prompt Cache`, it does not automatically include a Reference Loader fingerprint; include every relevant source and generation setting in `invalidate_key` yourself.

The Media header's **Snapshot** menu provides **Save** and **Load** actions for downloading and restoring a versioned JSON snapshot without queueing the workflow. A snapshot includes the complete Loader state, immutable image-original descriptors, structured Prompt document and Subject registry, Prompt preset, output-image controls, and caption visibility. **Load** validates the complete file before asking for confirmation and replacing the current Loader and Prompt settings. It does not contain media payloads or transient preview/editor history. Media **Clear** removes only media references; Prompt **Clear** immediately removes Prompt sections and Subjects while preserving the selected Prompt view and all Media.

Saving the workflow serializes versioned Loader and Prompt state into separate node widgets. Reloading restores card order, captions, toggles, edit recipes, trim ranges, display preferences, title-tag prompt sections, stable Subjects, and the selected prompt view. Older Prompt states are recovered as Raw, assigned v6 identities, and can be reviewed before switching to Structured; newer or malformed states are rejected. Undo/redo is available for board changes and inside each media editor; undo history itself is session-local.

## Load Reference Image

Add **Load Reference Image** when a workflow needs one ordinary IMAGE rather than a multi-reference bundle. Its visible surface stays close to ComfyUI's Load Image: one filename chooser with an adjacent **Edit** action and one preview. Double-clicking the empty preview opens the image chooser; after an image is loaded, double-clicking the preview opens Edit. The widget adds no custom node-width or node-height enforcement, and its preview explicitly overrides the shared card aspect ratio. Width and height can therefore be resized independently above ComfyUI's own native minima. The preview expands through all space below the control row; its image uses contain scaling in both dimensions, enlarging small sources and reducing large ones while preserving aspect ratio. Choosing or dropping another image replaces the current selection. The widget reuses Reference Loader's managed upload route, source validation, bounded WebP proxy, metadata, history, and image editing tools. Crop, flip, keep-mask painting, transparent/solid background, optional `rembg`, and Restore original therefore follow the same persistence and safety rules without a second editor or storage format. The single-image editor hides the shared caption field; the full Reference Loader continues to expose and persist captions in cards and editors.

The node outputs only RGB `image` and `mask`; it has no Prompt Advanced widget, description field, or description output. Preserve mode converts source/edit alpha to the Load Image-style inverted MASK while keeping IMAGE at three channels; images without alpha return a zero `[1,64,64]` mask. Opaque mode composites onto `alpha_background` before RGB output and returns the same zero-mask fallback. `limit_image_pixels` and `max_image_pixels` share Reference Loader's downscale-only execution path. Socketless `preview_pixels` accepts 0.25–16 MPixel, reloads the bounded proxy at that ceiling, and is excluded from the execution fingerprint. Source/edit identity and active execution image settings are fingerprinted. Exactly one enabled image is required when queueing. Animated GIF/WebP/TIFF inputs currently load their first frame rather than producing a batch.

The image chooser does not maintain a separate hard-coded extension allowlist. Upload inspection accepts image formats registered by the installed ComfyUI/Pillow runtime and preserves the original extension when it matches the decoded content. A decodable image with an incorrect image suffix is accepted and stored with a canonical extension—for example, JPEG bytes named `.png` become `.jpg`. Readable EXIF orientation is reflected consistently in metadata, previews, editing, and execution; malformed EXIF falls back to the stored pixel orientation without blocking the image.
...
Subjects are logical prompt references rather than media assets. Creating `#woman` stores a stable Subject ID and inserts an atomic chip; its compiled tag is derived from the Subject registry, such as `<Subject 1>`. Reusing `#woman` in another section points to the same stable ID. Section reordering preserves the registry's existing creation order, so dragging categories does not by itself renumber Subjects. Deleting a Subject may compact later numbers. A Subject can cite multiple `@` media mentions in its definition, and one media asset can define multiple Subjects. Subject labels are single tokens of up to 64 letters, numbers, underscores, or hyphens. When the last chip for a Subject is deleted, its unused registry entry is removed and no longer appears in the `#` picker. Raw recognizes `<Subject N>` only when that ordinal exists in the current registry; unknown Subject tags remain literal text.
...
The optional `response_seq` input accepts the `LLAMA_SEQUENTIAL_RESPONSE` custom type emitted by `Llama.cpp Sequential Generate`. It receives one object per media item in media-major order. Each object contains `request_index`, `kind`, `modality_index`, and the raw `response` string. With `response_format: text`, the response becomes `description`; with `response_format: json`, the response object must contain a string `description` and may contain additional string fields. All JSON fields are emitted under the matching `<Picture N>`, `<Video N>`, or `<Audio N>` mapping. `caption` and `source_video` are reserved and cannot be overridden. The older flat `response` STRING input remains supported, but `response` and `response_seq` cannot be connected together.
...
The **H3 Timeline Guides** editor is a top-level widget between Media and Subjects & Shots, collapsed and disabled by default. When enabled, it stores one `h3Timeline` object in `loader_state` with optional Image Start/End selections and up to 32 intermediate rows. Image and standalone Audio cards expose independent **G** Guide toggles and **G-pencil** actions beside their normal output controls; Video cards do not expose Guide controls. These Media actions remain the source-assignment entry point and focus the selected source in the separate Timeline widget. Each Shot card exposes the same **G-pencil** action; it opens the Timeline at the Shot's single frame and selects a matching Guide when present without adding a Shot-to-Guide field to serialized state. Each row has a stable ID, a non-negative output frame index at native 24 fps, and optional Image and standalone AUDIO selections. Reference Loader's general socketless `Frames` and `FPS` inputs persist the actual H3 output settings in `h3Output`; Timeline displays the same values read-only beside its summary, and displayed values are converted to native 24 fps integers when committed. The Wrapper forwards `Frames` as native `length` and rejects non-24 FPS until a compatible native model path exists. The editor can select disabled media cards because timeline sources are independent from ordinary reference output enablement.

Timeline sources are decoded into the bundle's runtime-only `guide_media` mapping. They are not added to `images`, `videos`, `audios`, caption lists, Prompt mentions, or native reference ordinals. An already-loaded active reference is reused when the same source is also a Guide. Source containment, size, hash, image edit, crop, and aggregate decoded-memory checks still apply. Video and video-derived Audio remain valid ordinary reference outputs but are rejected as Guide sources.

With Timeline enabled and at least one selected position, the H3 Wrapper delegates the base conditioning/AV latent to native `MiniMaxH3ReferenceToVideo`, resolves the actual latent frame count, and then calls native `MiniMaxH3AddGuide` in `(frame, stable ID)` order. Start uses `frame_idx=0`, ordinary rows use their stored frame, and End uses `frame_idx=-1`. Image guides require the video VAE; standalone Audio guides require the audio VAE. Visual ranges may not overlap other visual ranges, audio ranges may not overlap other audio ranges, and a visual plus audio guide at the same frame is allowed. Audio is cropped by the native node to the remaining AV timeline. Timeline OFF, or an enabled but empty timeline, retains the ordinary R2V path and does not require `MiniMaxH3AddGuide`.
...
- the VIDEO object retains the source container's embedded audio when **VA** is enabled;
- **VA** defaults to enabled for new videos and is stored as `enabled.video_audio` in the manifest; manifests from before this field existed default it to enabled;
- disabling **VA** materializes a VIDEO value from the public ComfyUI video components with `audio: null`, without changing the source file;
...
This makes both downstream choices available without duplicating sound by default: a newly added video's **V** and **VA** outputs start enabled and its separate **A** output starts disabled. Enable **A** only when a downstream consumer needs a separately decoded AUDIO value. A confirmed video without a decodable audio stream keeps **VA** and **A** unavailable.
...
```json
{
  ...
  "items": {
    ...
    "video-1": {
      ...
      "enabled": { "video": true, "video_audio": true, "audio": true }
    },
    "video-1:audio": {
      "kind": "audio",
      "source": {
...
- The custom DOM widget is shipped as a Bun browser ESM bootstrap plus a lazy implementation chunk under `dist/`; the React implementation is loaded when a Reference Loader custom widget is first created. It uses standard modern browser APIs, including modal dialogs, drag/drop, `AbortController`, canvas, and object URLs.
...
- VIDEO includes embedded audio by default while the Audio channel can emit the same soundtrack separately. Toggle **VA** to materialize a VIDEO without embedded audio; toggle **A** independently for the derived AUDIO value. This is deliberate, not automatic deduplication.
...
6. Preview an Audio card and confirm **▶** changes to **■**, Stop returns to the applied trim start, and starting another reference stops the first. Preview a VIDEO with **VA** on and confirm its embedded audio plays with the picture; toggle **VA** off and confirm the same picture preview is muted. Starting its Audio card stops the VIDEO and auditions the same range as audio-only regardless of **VA**. In **Edit**, verify the Videos-board editor follows **VA** while the Audio-board editor remains audible, then drag both trim handles, use the numeric fields and Seekbar before/during/after playback, audition the draft selection with the Play/Pause/Resume toggle and Stop, and verify Apply updates output duration while Cancel does not. Confirm playback is disabled for a silent video's **VA** and Audio controls, and the enabled video-derived AUDIO contains the matching trimmed range.
...
11. Open **Show advanced inputs**, change `grid_columns`, the float `preview_pixels` MPixel value, `show_captions`, `horizontal_cards`, `card_aspect`, `preview_fit`, and `waveform_pairs`. Confirm the card grid, horizontal/vertical card layout, proxy requests, image/video ratio and fit, and waveform density update while Audio remains 16:9; hidden captions remain editable through **Edit**, and none of the seven display fields changes execution outputs or the fingerprint. Change `prompt_schema_preset` across Generic, both H3 presets, and Freeform; confirm the badge, empty default section, and `/` menu update while existing sections, serialized prompt data, compiled output, and the fingerprint remain unchanged. In a Korean browser locale, confirm visible Prompt labels become Korean while preset IDs and title tags remain English. Then set `limit_image_pixels` to Limited and enter an exact `max_image_pixels` value below a large source's resolution. Confirm only the IMAGE tensor is downscaled, aspect ratio is retained, and small images are not enlarged. Set `composite_alpha` to Opaque and change `alpha_background`; confirm alpha-bearing outputs become three-channel RGB on that color while RGB inputs are unchanged. Confirm inactive max/background values do not change the fingerprint, while active values do.
12. Click Snapshot **Save**, inspect the downloaded version 2 JSON, then change the Media board, Prompt, preset, image-output controls, and caption visibility. Click **Load**, choose the snapshot, confirm replacement, and verify every saved value returns. Cancel a second load and verify the current state is unchanged. Try malformed, oversized, and wrong-version snapshots and verify each is rejected without partial replacement.
