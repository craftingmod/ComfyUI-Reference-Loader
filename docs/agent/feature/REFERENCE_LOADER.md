...
Saving the workflow serializes versioned Loader and Prompt state into separate node widgets. Reloading restores card order, captions, toggles, edit recipes, trim ranges, display preferences, title-tag prompt sections, stable Subjects, and the selected prompt view. Prompt state version 6 is the only supported persisted Prompt format; older, newer, or malformed states are rejected with a v6-only notice rather than migrated. Undo/redo is available for board changes and inside each media editor; undo history itself is session-local.
