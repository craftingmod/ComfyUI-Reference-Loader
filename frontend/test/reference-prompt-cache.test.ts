...
describe("Reference Prompt Cache frontend bridge", () => {
    ...
    eventApi.dispatchEvent(
      new CustomEvent("executed", {
        detail: {
          node: "1:7",
          display_node: "1:7",
          output: { cached_prompt: "nested prompt", cached_hash: "b".repeat(64) },
        },
      }),
    )
  ...
  test("keeps the cached prompt editable and locks the hash at the bottom", () => {
    const prompt = widget("cached_prompt")
    const hash = widget("cached_hash")
    const forceRefresh = widget("force_refresh")
    const node: ComfyNode = {
      type: REFERENCE_PROMPT_CACHE_NODE_TYPE,
      widgets: [prompt, hash, forceRefresh],
      addDOMWidget: () => prompt,
      setDirtyCanvas: () => undefined,
    }
    ...
    expect(node.widgets?.at(-1)?.name).toBe("cached_hash")
  })
})
