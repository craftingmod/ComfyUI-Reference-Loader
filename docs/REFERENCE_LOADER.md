...
Reference Loader appears under `reference / loader`; Reference Loader Export Prompt for LLM, Reference Loader Media Outputs, and Reference Loader Start/End Frames appear under `reference / output`; and MiniMax H3 Reference to Video Wrapper appears under `reference / integration`. A minimal saved graph is available as [`Reference_Loader.json`](../workflows/Reference_Loader.json).
