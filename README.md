...
- Optional top-level H3 Timeline Guides workspace between Media and Subjects & Shots for Start, End, up to 32 visual/audio guide rows, and an independent Shot lane; Media G buttons remain the source-assignment entry point and Shot cards expose matching G-pencil shortcuts into the Timeline at their frame, Reference Loader's general H3 Frames/FPS inputs are shown read-only in the Timeline, native H3 positions remain 24 fps, and guide-only media stays out of ordinary reference outputs
...
Connect it to **[Reference Loader] MiniMax H3 Wrapper** for native MiniMax H3 reference conditioning. The Wrapper exposes the native `clip`, `vae`, `audio_vae`, `prompt`, `width`, `height`, and `ref_image_size` controls and replaces the native `ref_*` Autogrow sockets with `references`; it reads H3 `Frames` and `FPS` from the bundle emitted by Reference Loader. Reference videos are decoded and sampled to the 24 fps IMAGE batch expected by MiniMax H3; no separate sampling node is required.

Use the top-level **H3 Timeline Guides** workspace between Media and Subjects & Shots to enable and edit the optional timeline. Image and standalone Audio cards expose independent **G** Guide toggles plus **G-pencil** actions beside the normal **I/A** output controls; Video cards intentionally expose no Guide controls. Media G-pencil actions remain the source-assignment entry point and focus the selected source in the separate Timeline workspace; each Shot card also exposes the same G-pencil control, which opens the Timeline at that Shot's single frame and selects a matching Guide when present. Start and End accept existing Images, while each guide row accepts an Image, a standalone Audio, or both, plus a non-negative output frame index. Prompt Shots appear in a separate **Shot** lane and keep their own tag/frame draft; they never enter H3 conditioning. Disabled ordinary references remain selectable, and Guide-only media is loaded into the bundle's separate `guide_media` mapping without changing reference ordinals, captions, or Prompt tags. Reference Loader's general `Frames` and `FPS` socketless inputs are the actual H3 output settings and are persisted in `loader_state`/the emitted bundle; Timeline displays them read-only beside its media/placement/shot summary. Saved Guide/Shot positions are converted to the native 24 fps integer frame at commit. The product limit is 32 intermediate guides; it is not a MiniMax H3 model limit.
...
The Media board and the top-level H3 Timeline workspace use their intrinsic content heights without accepting extra flex height, so neither gains a nested scrollbar. The compact Subjects & Shots Stack follows H3 Timeline, while Prompt receives the remaining node height.
