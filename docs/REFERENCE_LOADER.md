...
Reference Loader appears under `reference / loader`; [Reference Loader] Export Prompt for LLM, [Reference Loader] Media Outputs, and [Reference Loader] Start/End Frames appear under `reference / output`; and [Reference Loader] MiniMax H3 Wrapper appears under `reference / integration`. A minimal saved graph is available as [`Reference_Loader.json`](../workflows/Reference_Loader.json).
...
5. Prompt content is arranged as a stack of title-tag sections. Type `@` or `#` inside any section for a media mention or dialogue block. Section bodies and Raw use native multiline editing: `Enter` creates an ordinary new line and `Shift+Enter` follows the browser's standard `contenteditable` behavior. In the **Add section** row, enter a lowercase pseudo-YAML title such as `integrated_multimodal_description:` directly, or type `/` to choose an alias from the active prompt preset. Use **Raw** to inspect or edit the complete pseudo-YAML text.
...
The `/` commands are creation aliases supplied by the selected preset under **Show advanced inputs**. `generic` defaults to `scene:` and provides scene, style, camera, timeline, sound, music, voice, and avoid aliases. `minimax_h3_base` defaults to `integrated_multimodal_description:` and provides the exact H3 base description, soundscape, and music fields. `minimax_h3_reference` defaults to `detailed_description:` and provides the ordered H3 reference subject, summary, retention, description, soundscape, and music fields. `freeform` defaults to `scene:` but provides no aliases. A preset change never renames, reorders, or deletes existing sections; direct valid `title_tag:` input remains available in every preset. Presets affect neither compiled prompt output nor execution fingerprints.
...
1. Start ComfyUI, add `Reference Loader` and `[Reference Loader] Media Outputs`, connect `references`, and confirm the full DOM board appears without a console error.
