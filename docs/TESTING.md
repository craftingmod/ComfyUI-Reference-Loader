...
3. Reorder each board independently; toggle outputs and enter unique captions. Verify 1-based output badges have no gaps. Type `@` in Prompt, confirm image/video thumbnails appear, select mentions, and verify chips retain their media identity while their displayed ordinals follow reorder and enable changes. In Generic, create and reuse a `#name` Subject from any section. In MiniMax H3 Reference, verify creation is limited to `subject_definitions` and reuse works elsewhere. Confirm MiniMax H3 Base and Freeform keep `#` literal.
